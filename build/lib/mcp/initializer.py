...
class MCPInitializer:
    ...
    def __init__(self, root_path: Path, config_path: Optional[str] = None,
        ...
        if self.register_with_central:
            ...
            # Validate workspace name and description
            if not self.workspace_name:
                self.workspace_name = self.root_path.name
            if not self.workspace_description:
                if self.register_with_central:
                    raise ValueError("workspace_description is required when register_with_central=True")
                else:
                    self.workspace_description = f"MCP workspace for {self.workspace_name}"
